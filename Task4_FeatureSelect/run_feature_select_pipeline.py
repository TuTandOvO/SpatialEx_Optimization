#!/usr/bin/env python3
"""
Two feature-selection variants on top of SpatialEx:
  (A) SVG: keep only top-Moran's I genes in the loss; predict all genes at inference.
  (B) Attn: sigmoid gate on the H&E feature vector before the predictor MLP.

Set METHOD ∈ {'svg', 'attn', 'combined', 'ablation'} at the top to switch.
Note: SVG ended up hurting overall PCC; only the attention variant is harmless.
"""
